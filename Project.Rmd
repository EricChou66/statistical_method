...
#### Layer2
I found if the features includes square of transactions, the Std. Error is much larger than not included it.
```{r}
model.layer2 <- lm(Close ~ (Unique_Buyer + transactions + percentage), data = layer2_reg)
model.layer2$coefficients
```
#### Summary
```{r}
summary(model.layer2)$coefficients
#model.layer2$fitted.values
```
The Std. Error of percentage is higher that the other two features
Trying to take out the percentage features
```{r}
model.layer2 <- lm(Close ~ (Unique_Buyer + transactions), data = layer2_reg)
model.layer2$coefficients
```
#### Summary
```{r}
...
#### Layer6
```{r}
model.layer6 <- lm(Close ~ (Unique_Buyer + transactions + percentage), data = layer6_reg)
...
#model.layer6$fitted.values
```

Similar situation to layer2
```{r}
model.layer6 <- lm(Close ~ (Unique_Buyer + transactions), data = layer6_reg)
model.layer6$coefficients
```
...
the multiple regression has the lower Std. Error if the features contain Unique_Buyer and transactions